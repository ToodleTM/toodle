...
angular.module('toodleApp')
    .controller('NavbarCtrl', function ($scope, $location, $translate, $cookies, $cookieStore) {
        ...
        $scope.switchLanguage = function(newLang){
            ...
            $('#activeLanguage').attr('src', './images/flags/'+$scope.selectedLanguage+'-'+$scope.selectedLanguage.toUpperCase()+'.svg');
            console.log($('#activeLanguage').attr('src'));
        };
        $scope.switchLanguage($scope.selectedLanguage);

        $('#languageSelector ul li a').click(function(e){
            e.preventDefault();
            $scope.switchLanguage($(this).attr('language'));$scope.$apply();
        });
    });
