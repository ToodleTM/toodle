...
SingleElim.prototype.updateBracketData = function(bracket, callback){
    var targetRound = (_.keys(bracket).length +1) / 2;
    _.each(bracket, function(match){
        if((!match.player1 || !match.player2) && match.round === targetRound){
            match.complete = true;
        }
    });
