...
SingleElim.prototype.meta = {version: '1.1', description: 'Single elimination bracket', compatible: ['playerSwap'], name:'singleElim'};
...
SingleElim.prototype.swapPlayers = function(match1, playerToSwapInMatch1, match2, playerToSwapInMatch2, bracket, callback){
    if(match1 && match2){
        if(!match1.complete && !match2.complete){
            if(bracket[match1.number].player1 && bracket[match1.number].player2 && !match2[playerToSwapInMatch2]){
                bracket[match1.number].complete = true;
            }
            if(bracket[match2.number].player1 && bracket[match2.number].player2 && !match1[playerToSwapInMatch1]){
                bracket[match2.number].complete = true;
            }
