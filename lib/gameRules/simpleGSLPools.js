...
function isFirstMatchOfRound1(match) {
    return match.round === 1 && match.number % NUMBER_OF_MATCHES_IN_GROUP === 1;
}
function isSecondMatchOfRound1(match) {
    return match.round === 1 && match.number % NUMBER_OF_MATCHES_IN_GROUP === 2;
}
function isWinnersMatchOfRound2(match, matchNumber) {
    return match.round === 2 && matchNumber % NUMBER_OF_MATCHES_IN_GROUP === 3;
}
function isLosersMatchOfRound2(match, matchNumber) {
    return match.round === 2 && matchNumber % NUMBER_OF_MATCHES_IN_GROUP === 4;
}
        ...
        if (score1 !== score2) {
            ...
            if (match) {
                if (tryingToReportRound2WhenRound1IsNotOver(matchNumberInGroup, groups, groupNumber)) {
                    ...
                    } else if (isWinnersMatchOfRound2(match, matchNumber)) {
                        seedRound2WinnerMatchLoserIntoDeciderMatch(groups, groupNumber, loser);
                    } else if (isLosersMatchOfRound2(match, matchNumber)) {
