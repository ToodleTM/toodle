...
ServerUtils.prototype.handleMultipleSeeding = function (parsedCSV, TournamentModel, req, res, tournamentService) {
    if (parsedCSV && parsedCSV[0].name) {
        TournamentModel.findById(req.query.tournamentId, function (err, tournamentData) {
            if (err) {
                return res.status(NOT_FOUND).json({message:'noSuchTournament'});
            } else {
                try {
                    ...
                    return res.status(CLIENT_ERROR).json({message: 'notACSVFile'});
                }
            }
        });
    } else {
        res.status(CLIENT_ERROR).json({message: 'noNameField'});
    }
};
