    ...
    if (currentMatch) {
        if (currentMatch.complete) {
            if (currentMatch.player1 && currentMatch.player2) {
                ...
                delete currentMatch.forfeit;
                delete currentMatch.winner;
...
SingleElim.prototype.forfeit = function (matchNumber, forfeitSlot, score1, score2, bracket, callback) {
    var matchToUpdate = bracket[matchNumber];
    if (matchToUpdate) {
        if (matchToUpdate.complete) {
            callback({message: 'cantDefwinMatchIsComplete'});
        } else {
            if (forfeitSlot === 1 || forfeitSlot === 2) {
                matchToUpdate.complete = true;
                matchToUpdate.forfeit = true;
                matchToUpdate.winner = forfeitSlot === 1 ? 2 : 1;
                matchToUpdate.score1 = score1;
                matchToUpdate.score2 = score2;
                if (matchToUpdate.next) {
                    var upcomingMatchSlot = matchToUpdate.nextFirst ? 'player1' : 'player2';
                    bracket[matchToUpdate.next][upcomingMatchSlot] = matchToUpdate['player' + matchToUpdate.winner];
                }
                callback(null, bracket);
            } else {
...
SingleElim.prototype.getLastmatchInTournament = function (tournament) {
    return tournament.bracket[_.keys(tournament.bracket).length];
};
...
SingleElim.prototype.winners = function (tournament, callback) {
    ...
    if (tournament.running) {
        if (matchesToReport.length) {
            ...
            var winnerMatch = this.getLastmatchInTournament(tournament);
...
SingleElim.prototype.swapPlayers = function (match1, playerToSwapInMatch1, match2, playerToSwapInMatch2, bracket, callback) {
    if (match1 && match2) {
        var firstPlayer = bracket[match1]['player' + playerToSwapInMatch1];
        var secondPlayer = bracket[match2]['player' +playerToSwapInMatch2];
        bracket[match1]['player' + playerToSwapInMatch1] = secondPlayer;
        bracket[match2]['player' + playerToSwapInMatch2] = firstPlayer;
        callback(null, bracket);
    } else {
