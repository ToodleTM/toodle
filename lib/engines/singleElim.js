...
function isFirstRoundMatchThatMisesAPlayer(match, targetRound) {
    return (!match.player1 || !match.player2) && match.round === targetRound;
}
...
function nextMatchSlotIsEmpty(match, bracket){
    var playerSlot = match.nextFirst ? 'player1' : 'player2';
    return match.next && !bracket[match.next][playerSlot];
}
SingleElim.prototype.updateBracketMatchesStatusesAndStandings = function (bracket, callback) {
    var targetRound = _.keys(bracket).length + 1;
    _.each(bracket, function (match) {
        if (nextMatchSlotIsFull(match, bracket)) {
            match.complete = true;
        }
        if (isFirstRoundMatchThatMisesAPlayer(match, targetRound) && nextMatchSlotIsEmpty(match, bracket)) {
