...
var utils = new (require('..//utils/serverUtils'))();
...
TournamentService.prototype.saveTournament = function (req, res, tournament) {
    tournament.signupID = utils.createTournamentId(new Date().getTime(), req.body.tournamentName);
...
TournamentService.prototype.reportMatch = function (req, res, tournament, TournamentModel) {
    ...
    if(tournament.running){
        try {
            ...
            return res.json(500, {message:'errorReportingMatch'});
        }
    } else {
        res.json(409, {message:'tournamentNotRunning'});
    }
};
...
TournamentService.prototype.unreportMatch = function (req, res, tournament, TournamentModel) {
    ...
    if(tournament.running){
        try {
            ...
            return res.json(500, {message:'errorUnreportingMatch'});
        }
    } else {
        res.json(409, {message:'tournamentNotRunning'});
    }
    ...
    if (!tournament.running) {
        ...
        if (playerToMove) {
            if (targetPlayer || target === null) {
                ...
                return res.json(409, {message: 'playerToMoveNextToDoesNotExist'});
            }
        } else {
            return res.json(tournament);
        }
    } else {
        return res.json(409, {message: 'unableToChangeOrderWhileTournamentIsLive'});
    }
};
