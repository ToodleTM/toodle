...
TournamentService.prototype.multipleSeed = function (req, res, tournament, TournamentModel, playersToAdd) {
    var uniquePlayersToAdd = _.uniq(playersToAdd, function (player) {
        return player.name.toLowerCase().trim()
    });
    var alreadyEnlistedPlayers = _.forEach(tournament.players, function(player){return {name:player.name.toLowerCase().trim()}});

    _.forEach(uniquePlayersToAdd, function (playerToAdd) {
        if(alreadyEnlistedPlayers.length == 0  || !_.indexOf(alreadyEnlistedPlayers, playerToAdd.name.toLowerCase().trim())){
            tournament.players.push({name: playerToAdd.name.trim()});
        }
    });
