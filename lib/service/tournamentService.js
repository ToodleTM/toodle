...
TournamentService.prototype.registerPlayer = function (req, res, tournament, lockingOverride) {
    ...
    if (tournament.running) {
        ...
        return res.json(SERVER_APP_ERROR, {message: 'noDuplicateNick'});
    } else if (tournament.locked && !lockingOverride) {
        return res.json(SERVER_APP_ERROR, {message: 'registrationLocked'});
        ...
        return res.json(SERVER_APP_ERROR, {message: 'noEmptyNick'});
    }
};
        ...
        if (tournament.engine && tournament.engine !== 'none') {
            ...
            if (engine) {
                if(engine.meta.compatible.indexOf('playerSwap') !== -1) {
                    ...
                    res.json(SERVER_APP_ERROR, {message:'incompatibleEngine'});
                }
            } else {
...
TournamentService.prototype.updateBracketDataAndStart = function(req, res, tournament, TournamentModel){
    if (tournament.players && tournament.players.length) {
        if (tournament.engine && tournament.engine !== 'none') {
            ...
            if (engine) {
                engine.updateBracketData(tournament.bracket, function (err, updatedBracket) {
                    if (err) {
                        return res.json(SERVER_APP_ERROR, err);
                    } else {
                        ...
                        TournamentModel.update({signupID: tournament.signupID}, {bracket: tournament.bracket, running:true, locked:true}, function (err) {
                            if (err) {
...
TournamentService.prototype.swapPlayers = function (req, res, tournament, TournamentModel) {
    ...
    if (this.areSwapParametersValid(req.body.playerInMatch1, req.body.playerInMatch2)) {
        ...
        if (matchesContainingPlayersToSwap.length > 1) {
            var playerToSwapInMatch1 = req.body.playerInMatch1.isPlayer1 ? 'player1' : 'player2';
            var playerToSwapInMatch2 = req.body.playerInMatch2.isPlayer1 ? 'player1' : 'player2';
            ...
            res.json(SERVER_APP_ERROR, {message: 'noSwappablePlayersFound'});
        }
    } else {
        res.json(SERVER_APP_ERROR, {message: 'errorWhenGettingPlayersToSwap'});
    }
};
