...
TournamentService.prototype.swapPlayers = function (req, res, tournament, TournamentModel) {
    ...
    engine.getMatchesToReport(tournament.bracket, function(err, matchesToReport){
        if(!err) {
            var playersToSwap = _.filter(matchesToReport, function (match) {
                return match.player1.name === req.body.playerInMatch1.name || match.player2.name === req.body.playerInMatch1.name ||
                    match.player1.name === req.body.playerInMatch2.name || match.player2.name === req.body.playerInMatch2.name;
            });
            if(playersToSwap.length > 1){
                var playerToSwapInMatch1 = playersToSwap[0].player1.name === req.body.playerInMatch1.name ? 'player1' : 'player2';
                var playerToSwapInMatch2 = playersToSwap[1].player1.name === req.body.playerInMatch2.name ? 'player1' : 'player2';

                engine.swapPlayers(playersToSwap[0], playerToSwapInMatch1, playersToSwap[1], playerToSwapInMatch2, tournament.bracket, function (err, data) {
                    if (err) {
                        res.json(SERVER_APP_ERROR, err);
                    } else {
                        tournament.bracket = data;
