...
TournamentService.prototype.multipleSeed = function (req, res, tournament, playersToAdd) {
    ...
    _.forEach(uniquePlayersToAdd, function (playerToAdd) {
        if(alreadyEnlistedPlayers.length == 0  || !_.find(alreadyEnlistedPlayers, function(item){return item.name.toLowerCase() == playerToAdd.name.toLowerCase().trim()})){
            tournament.players.push({name: playerToAdd.name.trim(), faction:playerToAdd.faction.trim()});
        }
    });
