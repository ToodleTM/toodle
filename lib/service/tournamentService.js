...
TournamentService.prototype.reportMatch = function(req, res, tournament){
    var engine = this.getTournamentEngine(tournament.engine);
    try {
        engine.reportWin(req.body.number, req.body.score1, req.body.score2, tournament.bracket, function(err, data){
            if(err){
                ...
                Tournament.update({_id: tournament._id}, {bracket:data}, function(err){
                    if(err){
                        return res.json(500, 'errorReportingMatch');
                    } else {
                        return res.json(tournament);
                ...
                //return that.updateTournament(req, res, tournament);
            }
        });
    } catch (exception){
        return res.json(500, 'errorReportingMatch');
